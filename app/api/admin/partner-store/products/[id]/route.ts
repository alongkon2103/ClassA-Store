...
//   ร้านแบบ maki_api เพิ่ม: ชื่อ TH/EN, คำอธิบาย, รูปปก/แกลเลอรี, ราคาขายต่อแพลน (ต้อง ≥ ขั้นต่ำสดจาก Maki), โปรแกรมที่ต้องโหลด (downloads [{name,url}]), วิดีโอ YouTube หลังซื้อ (guide_videos [{title,url}]), รูปฟังก์ชันหน้าสร้างรูปไลฟ์ (livegen_functions [{name,image_url}])
...
import { getMakiCatalog, toPlanRows, toDownloads, toGuideVideos, toLivegenFunctions, priceFrom, asJson } from "@/lib/maki"
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isMaki) {
    ...
    if (Array.isArray(body.guide_videos)) data.guide_videos = asJson(toGuideVideos(body.guide_videos))
