...
export async function PATCH(req: Request, { params }: { params: Promise<{ fid: string }> }) {
    const session = await getServerSession(authOptions)
    if (session?.user?.role !== "admin") return NextResponse.json({ error: "Forbidden" }, { status: 403 })

    const { fid } = await params  // ← await params
    const { name, label_th, label_en } = await req.json()
    const fn = await prisma.product_functions.update({
        where: { id: fid },
        data: { ...(name && { name }), label_th: label_th ?? null, label_en: label_en ?? null },
    })
