...
const STATUS: Record<string, number> = { not_found: 404, unavailable: 400, no_identity: 400, onboarding: 503, below_min: 409, maki_error: 502 }
...
export async function POST(req: NextRequest) {
  ...
  try {
    const r = await createMakiCheckout({ userId: session.user.id, provider: session.user.provider, partnerProductId: productId, planKey, locale: String(body.locale ?? "th") })
    ...
    if (e instanceof MakiCheckoutError) {
      ...
      return NextResponse.json({ error: e.code }, { status: STATUS[e.code] ?? 500 })
    }
