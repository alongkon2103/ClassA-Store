...
export default async function Page({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const [product, points] = await Promise.all([
    prisma.partner_products.findUnique({ where: { id: row.partner_product_id }, select: { external_slug: true, name_th: true, name_en: true, thumbnail_url: true, images: true, plans: true } }),
