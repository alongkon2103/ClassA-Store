        ...
        return (
            <div className="min-h-screen bg-bg-base">
                ...
                <div className="flex flex-col items-center justify-center py-20 px-6">
                    ...
                    <p className="text-text-muted mb-8 text-center">We couldn't find the order you're looking for.</p>
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 md:py-12">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 md:mb-10">
                    ...
                    <div className="md:text-right self-start md:self-auto">
                        <p className="text-text-muted text-[10px] md:text-[11px] uppercase tracking-wider mb-1">Transaction ID</p>
                        <p className="font-mono text-[12px] text-white/80 bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 break-all">
                            {order.id}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
                    ...
                    <div className="lg:col-span-8 space-y-6 md:space-y-8">
                        ...
                        <div className="relative group">
                            ...
                            <div className="relative bg-bg-card border border-white/5 rounded-2xl md:rounded-3xl p-5 md:p-8 overflow-hidden">
                                ...
                                {order.status === "paid" ? (
                                    <div className="space-y-4">
                                        <div className="bg-bg-base/50 border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6 text-center group/key relative overflow-hidden">
                                            <p className="font-mono text-lg md:text-2xl font-bold tracking-wider text-accent-light break-all relative z-10">
                                                {order.game_keys?.key_value || "Processing Key..."}
                                            </p>
                                        </div>
                                        <p className="text-[12px] text-text-muted text-center italic">
                                            Copy this key and activate it on the respective platform.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="py-10 text-center">
                                        <div className="animate-spin w-8 h-8 border-2 border-accent border-t-transparent rounded-full mx-auto mb-4"></div>
                                        <p className="text-text-muted text-[14px]">Waiting for confirmation...</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {order.status === "paid" && (order.products.product_gifts.length > 0 || order.products.product_presets.length > 0) && (
                            <div className="bg-bg-card border border-white/5 rounded-2xl md:rounded-3xl p-5 md:p-8">
                                <h2 className="text-lg font-bold text-white mb-6 md:mb-8 flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    {order.products.product_gifts.map((gift) => (
                                        <div key={gift.id} className="flex flex-col bg-bg-base/30 border border-white/5 rounded-xl p-4 transition-colors hover:border-violet-500/30">
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className="w-10 h-10 rounded-lg bg-violet-500/10 flex items-center justify-center text-[18px]">
                                                    <ImageIcon />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[13px] font-bold text-white truncate">{gift.filename || "Media Asset"}</p>
                                                    <p className="text-[10px] text-text-muted uppercase tracking-wider">Image Asset</p>
                                                </div>
                                            </div>
                                            <a href={gift.url} target="_blank" rel="noopener noreferrer" className="mt-auto w-full py-2 bg-white/5 hover:bg-violet-600 text-white text-[12px] font-semibold rounded-lg text-center transition">
                                                Download
                                            </a>
                                        </div>
                                    ))}
                                    ...
                                    {order.products.product_presets.map((preset) => (
                                        <div key={preset.id} className="flex flex-col bg-bg-base/30 border border-white/5 rounded-xl p-4 transition-colors hover:border-blue-500/30">
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center text-[18px]">
                                                    ⚙️
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[13px] font-bold text-white truncate">{preset.filename || "Config"}</p>
                                                    <p className="text-[10px] text-text-muted uppercase tracking-wider">Preset</p>
                                                </div>
                                            </div>
                                            <a href={preset.url} target="_blank" rel="noopener noreferrer" className="mt-auto w-full py-2 bg-white/5 hover:bg-blue-600 text-white text-[12px] font-semibold rounded-lg text-center transition">
                                                Download
                                            </a>
                                        </div>
                                    ))}
                    ...
                    <div className="lg:col-span-4 space-y-4 md:space-y-6">
                        <div className="bg-bg-card border border-white/5 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl">
                            <div className="relative aspect-[21/9] lg:aspect-video">
                                <Image src={mainImage} alt={order.products.name_en} fill className="object-cover" />
                                <div className="absolute inset-0 bg-gradient-to-t from-bg-card via-transparent to-transparent"></div>
                            </div>
                            <div className="p-5 md:p-6">
                                <h3 className="text-[16px] md:text-[18px] font-bold text-white mb-1">
                                    {order.products.name_en}
                                </h3>
                                <p className="text-accent-light text-[13px] font-medium mb-4">
                                    {order.product_variants?.label_en || "Standard Version"}
                                </p>
                        ...
                        <Link href="/orders" className="flex items-center justify-center gap-2 w-full py-3 border border-white/10 text-text-muted hover:text-white text-[12px] font-medium rounded-xl transition">
